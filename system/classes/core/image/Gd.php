 ...
 * @category   Drivers
 */
class Core_Image_GD extends Image {
    ...
    public static function check()
    {
        ...
        {
            throw new Core_Exception('GD is either not installed or not enabled, check your configuration');
        }
        ...
        {
            throw new Core_Exception('Image_GD requires GD version :required or greater, you have :version',
    ...
    public function __construct($file)
    {
        ...
        if ( ! isset($create) OR ! function_exists($create))
        {
            if(Core::$selected_mode == Core::DEVELOPMENT){
                throw new Core_Exception('Installed GD does not support :type images',
                    array(':type' => image_type_to_extension($this->type, FALSE)));
            }else{
                // Если ошибка у обычного пользователя
                // Get the real path to the file
                $file = realpath(dirname(__FILE__)).DIRECTORY_SEPARATOR.'error.png';
    ...
    protected function _do_rotate($degrees)
    {
        ...
        {
            throw new Core_Exception('This method requires :function, which is only available in the bundled version of GD',
