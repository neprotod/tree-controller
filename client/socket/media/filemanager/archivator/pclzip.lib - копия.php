...
//      if (is_file($p_filename))
      if (   ($p_filedescr['type'] == 'file')
        ...
        if ($p_filedescr['type'] == 'file') {        
          ...
          $big_zip = FALSE;
          if($p_header['size'] > 10000000 AND memory_get_usage() > 300000){
            $big_zip = TRUE;
          }else{
        ...
        // ----- Calculate the CRC
        $checksum = hash_file('crc32b',$p_filename,true);
        $unpack = unpack("N*", $checksum);
        if($unpack = reset($unpack) OR $unpack === 0)
            $p_header['crc'] = $unpack;
        else
            exit('Ошибка');

        //echo $p_header['crc'] . '<br>';
        // ----- Look for no compression
        if ($p_options[PCLZIP_OPT_NO_COMPRESSION] OR  $big_zip === TRUE) {
        ...
        */
        ...
        if($big_zip == FALSE){
            ...
            $v_file = @fopen($p_filename, "rb");
            while(!feof($v_file)){
                $v_content = fread($v_file, 10000000);
                @fwrite($this->zip_fd, $v_content, $p_header['compressed_size']);
            }
